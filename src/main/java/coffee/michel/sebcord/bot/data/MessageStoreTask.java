 ...
 */
package coffee.michel.sebcord.bot.data;
...
//			return;
		...
		exe.submit(() -> {
			...
			List<TextChannel> allTextChannels = guild.getCategories().stream().flatMap(c -> c.getChannels().stream()).filter(TextChannel.class::isInstance).map(TextChannel.class::cast).sorted((c1, c2) -> c2.getTimeCreated().compareTo(c1.getTimeCreated())).collect(Collectors.toList());
			for (TextChannel channel : allTextChannels) {
				final var channelId = channel.getIdLong();

				exe.submit(() -> {
					Iterator<Message> iterator = channel.getHistoryFromBeginning(100).complete().getRetrievedHistory().iterator();
					Message message = null;
					while (true) {
						if (!iterator.hasNext()) {
							if (message != null) {
								iterator = channel.getHistoryAfter(message, 100).complete().getRetrievedHistory().iterator();
								continue;
							} else
								break;
						}
	...
	private void store(long channelId, Message next) {
		...
		data.emotes = Lazy.Reference(next.getEmotes().stream().map(e -> new EmoteData(e.getName(), e.getImageUrl())).collect(Collectors.toList()));
		...
		data.reactions = Lazy.Reference(next.getReactions().stream().map(r -> {
			...
			if (reactionEmote.isEmote()) {
				...
				return new ReactionData(new EmoteData(emote.getName(), emote.getImageUrl()), r.hasCount() ? r.getCount() : 0, null);
			} else {
				return new ReactionData(reactionEmote.getEmoji(), r.hasCount() ? r.getCount() : 0, null);
			}
		}).collect(Collectors.toList()));

		data.channelMentions = Lazy.Reference(next.getMentionedChannels().stream().map(c -> new Tuple<>(c.getName(), c.getIdLong())).collect(Collectors.toSet()));
		data.roleMentions = Lazy.Reference(next.getMentionedRoles().stream().map(r -> new Tuple<>(r.getName(), r.getIdLong())).collect(Collectors.toSet()));
		data.userMentions = Lazy.Reference(next.getMentionedUsers().stream().map(r -> new Tuple<>(r.getName(), r.getIdLong())).collect(Collectors.toSet()));
		data.memberMentions = Lazy.Reference(next.getMentionedMembers().stream().map(r -> new Tuple<>(r.getEffectiveName(), r.getIdLong())).collect(Collectors.toSet()));
	...
	public static class MessageData {
		...
		public String loadedToString() {
			...
			builder.append("MessageData [id=")
					...
					.append(content)
					.append(", embedds=")
					.append(embedds.stream().map(l -> l.get()).map(MessageEmbed::toString).collect(Collectors.joining("; ")))
					.append(", emotes=")
