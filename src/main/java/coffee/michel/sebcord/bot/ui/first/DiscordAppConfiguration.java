...
public class DiscordAppConfiguration extends VerticalLayout {
	...
	private void initUI() {
		...
		Anchor forwardToApplicationOverview = new Anchor("https://discordapp.com/developers/applications", new Icon(VaadinIcon.ARROW_FORWARD));
		...
		saveButton.addClickListener(ce -> {
			discordApp.setClientId(Optional.ofNullable(clientIdField.getValue()).filter(s -> !s.isEmpty()).map(Long::valueOf).orElse(0L));
			...
			persistence.getBotConfig().setHandledServerId(Optional.ofNullable(handlesServerId.getValue()).map(Long::valueOf).orElse(0L));
